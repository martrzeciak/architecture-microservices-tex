...
W~rozdziale zestawiono prace empiryczne porównujące REST z~gRPC lub GraphQL, aby określić, które elementy komunikacji zostały już zbadane i~w~jakim stopniu opublikowane wyniki odnoszą się do~aplikacji działającej w~przeglądarce. Dokumentacje techniczne, wykorzystywane w~dalszych rozdziałach, służą wyjaśnieniu mechanizmów implementacyjnych; ocenę wydajności oparto wyłącznie na~badaniach eksperymentalnych.
...
Omówione prace obejmują REST, gRPC i~GraphQL mierzone po~stronie usługi. Poza ich zakresem pozostaje gRPC-Web w~dwóch wariantach obsługi, przez serwer pośredniczący Envoy oraz bezpośrednio w~ASP.NET Core, a~także pomiar domykany aktualizacją drzewa dokumentu. Taki właśnie układ jest przedmiotem eksperymentu opisanego w~rozdziałach~\ref{cha:MetodologiaBadan} i~\ref{cha:AnalizaWydajnosci}: trzy konfiguracje komunikacyjne, jedna aplikacja kliencka, kontrolowany rozmiar odpowiedzi oraz rozróżnienie ścieżek dostępu do~danych. Uzyskane wyniki zestawiono z~omówionymi tu~pracami w~sekcji~\ref{sec:ZestawienieZBadaniami}. GraphQL potraktowano jako alternatywny sposób doboru danych i~pozostawiono poza zakresem eksperymentu, ponieważ dobór pól przez klienta zmieniałby objętość odpowiedzi niezależnie od~sposobu jej kodowania.
