...
Platforma testowa, której architekturę i~wdrożenie opisano w~rozdziale~\ref{cha:StudiumPrzypadku}, wymagała uzupełnienia o~rozwiązania służące samej procedurze badawczej. Wprowadzone modyfikacje mają wspólny cel: utrzymać stałe wszystkie czynniki poza badanym protokołem, tak aby zmierzoną różnicę można było przypisać sposobowi komunikacji.
...
Każda ścieżka przechowuje w~buforze reprezentację odpowiadającą formatowi swojej odpowiedzi, co~opisano w~rozdziale~\ref{cha:StudiumPrzypadku}. Dla pomiaru istotna jest wynikająca z~tego różnica kosztu obsługi trafienia. Ścieżka REST zwraca zapisany dokument bez ponownego przetwarzania. Ścieżka gRPC odtwarza komunikat z~bajtów i~serializuje go~do~odpowiedzi. Warianty Envoy oraz Direct korzystają z~tej~samej implementacji usługi, wobec czego obciążenie to~dotyczy ich w~równym stopniu. Różnicy nie da~się usunąć bez zmiany jednej z~implementacji, dlatego uwzględniono ją~w~ograniczeniach w~sekcji~\ref{sec:OgraniczeniaMetodologiczne}, a~scenariusz Echo pozostawiono jako punkt odniesienia bez udziału pamięci podręcznej.

\paragraph{Dobór wolumenu danych testowych}
Baza danych katalogu produktów została zainicjalizowana zbiorem 2000 rekordów. Wartość ta~wyznacza górną granicę rozmiaru strony wyników, co~umożliwia generowanie odpowiedzi o~rozmiarze przekraczającym 260~kB. Taki zakres pozwala badać zachowanie protokołów zarówno dla odpowiedzi mieszczących się w~jednym oknie odbiorczym, jak i~dla odpowiedzi wymagających kilku kolejnych obiegów pakietu; mechanizm ten omówiono w~rozdziale~\ref{cha:AnalizaWydajnosci}.
...
\textbf{Rotacja kolejności protokołów.} Kolejność badania protokołów rotuje cyklicznie pomiędzy przebiegami, wobec czego każdy z~nich bywa mierzony jako pierwszy, drugi i~ostatni. Przy trzech protokołach i~pięciu przebiegach rotacja nie równoważy kolejności w~pełni, ogranicza jednak ryzyko systematycznego uprzywilejowania protokołu badanego po~osiągnięciu przez system stanu ustalonego. W~literaturze zjawisko to~określa się jako efekt kolejności~\cite{bib:Jain1991}.
...
Zestawienie wymiarów daje 44~kombinacje, z~których pięć odrzucono, ponieważ iloczyn liczby równoległych klientów i~liczby rekordów w~odpowiedzi przekraczał przyjęty próg 25~tysięcy. Powyżej tej wartości czynnikiem ograniczającym przestaje być protokół, a~staje się warstwa danych albo procesor serwera. Odrzucone kombinacje to~scenariusz Echo dla 5000~rekordów przy obu poziomach obciążenia oraz dla 2000~rekordów przy dwudziestu klientach, a~także scenariusz Products dla strony 2000~produktów przy dwudziestu klientach, w~obu stanach pamięci podręcznej.

Zmierzono ostatecznie 39~konfiguracji, co~przy trzech protokołach i~pięciu powtórzeniach daje 585~wykonanych testów oraz 171~tysięcy zarejestrowanych pomiarów. Łączny czas przebiegu wyniósł 69,7~minuty.
...
gdzie $t$ oznacza wartość krytyczną rozkładu Studenta dla czterech stopni swobody. Przy pięciu obserwacjach mnożnik ten jest znacznie większy od~wartości granicznej 1,96, co~poszerza przedział i~utrudnia uznanie różnicy za~rozstrzygniętą na~podstawie małej próby.
