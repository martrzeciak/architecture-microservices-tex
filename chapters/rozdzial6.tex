...
Zestawienie wymiarów daje 44~kombinacje, z~których pięć pominięto, ponieważ iloczyn liczby równoległych klientów i~liczby rekordów w~odpowiedzi przekraczał przyjęty operacyjnie próg 25~tysięcy. Ograniczenie to~zastosowano, aby zmniejszyć wpływ nasycenia zasobów w~największych konfiguracjach. Pominięte kombinacje to~scenariusz Echo dla 5000~rekordów przy obu poziomach obciążenia oraz dla 2000~rekordów przy dwudziestu klientach, a~także scenariusz Products dla strony 2000~produktów przy dwudziestu klientach, w~obu stanach pamięci podręcznej.

Zmierzono ostatecznie 37~konfiguracji, co~przy trzech wariantach komunikacji i~pięciu powtórzeniach daje 555~wykonanych testów oraz około 133~tysięcy zarejestrowanych pomiarów. Łączny czas przebiegu wyniósł 69,7~minuty.
