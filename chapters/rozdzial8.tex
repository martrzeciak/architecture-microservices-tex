...
System eShop demonstruje praktyczne zastosowanie wzorców opisanych w~rozdziale~\ref{cha:WzorceProjektowe}: Cache-Aside, Transactional Outbox, Saga, CQRS z~Minimal API oraz strumieniowania serwerowego.
...
\subsection{CQRS z~Minimal API}
\label{subsec:CQRSImpl}

System realizuje wzorzec CQRS~\cite{bib:YoungCQRS} w~zakresie omówionym w~sekcji~\ref{sec:CQRS}, to~jest poprzez separację ścieżek odczytu i~zapisu na~poziomie mikroserwisów, bez~odrębnego modelu odczytu utrzymywanego projekcjami. Ścieżka odczytu w~\texttt{ProductService} optymalizowana jest pod~kątem wydajności: przez wyłączenie śledzenia zmian, wzorzec Cache-Aside, paginację oraz pulę kontekstów bazodanowych. Ścieżka zapisu w~\texttt{OrderService} optymalizowana jest natomiast pod~kątem spójności, przez transakcje, wzorzec Outbox oraz maszynę stanów.

Model Minimal API pozwala pominąć warstwę kontrolerów~\cite{bib:MinimalApi}: każdy punkt końcowy definiowany jest jako wyrażenie lambda rejestrowane bezpośrednio w~obiekcie aplikacji, wraz z~jawnie wskazanymi zależnościami. Upraszcza to~kod oraz skraca ścieżkę przetwarzania żądania o~mechanizmy wiązania właściwe kontrolerom; wpływu tej różnicy na~czas odpowiedzi nie mierzono, ponieważ jest ona wspólna dla wszystkich badanych ścieżek komunikacji.

Wyniki eksperymentu wskazują, że~separacja ta~ma~uzasadnienie empiryczne: obie ścieżki wykazują nie tylko odmienne wartości opóźnień, lecz przede wszystkim odmienną charakterystykę: ścieżka odczytu reaguje na~zmianę protokołu komunikacji, ścieżka zapisu pozostaje na~nią~niewrażliwa (sekcja~\ref{sec:AnalizaOrders}).
