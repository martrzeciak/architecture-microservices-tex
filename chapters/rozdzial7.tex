...
Wyniki eksperymentu przedstawiono zgodnie z~metodologią opisaną w~rozdziale~\ref{cha:MetodologiaBadan}. Analiza obejmuje wolumen przesyłanych danych, model liczby wymian pakietów, scenariusze Echo, Products i~Orders, porównanie konfiguracji komunikacyjnych, granicę przepustowości stanowiska pomiarowego, przedziały ufności oraz weryfikację hipotez.
...
Kompresja redukuje wolumen formatu tekstowego 12--13-krotnie, a~formatu binarnego 9,5--11-krotnie. Po~jej zastosowaniu przewaga objętościowa serializacji binarnej maleje z~33\% do~15--18\%. Wszystkie badane ładunki mieszczą się wówczas w~jednym oknie odbiorczym. Przeniesienie wyników dla transferów nieskompresowanych na~wdrożenie z~włączoną kompresją wymagałoby odrębnego pomiaru.
...
Do~200~rekordów włącznie różnice pomiędzy protokołami pozostają niewielkie, od~trzech do~piętnastu procent, przy wartościach bezwzględnych rzędu jednej do~pięciu milisekund. Zgodnie z~modelem z~sekcji~\ref{sec:ModelOkna} odpowiedzi tego rozmiaru mieszczą się w~jednym oknie w~obu formatach; liczba obiegów pakietu jest zatem identyczna, a~pozostała różnica wynika z~narzutu ramkowania i~warstwy transportowej.

Przejście z~200 na~500~rekordów powoduje skokową zmianę obrazu. Opóźnienie REST rośnie z~32,6 do~59,8~ms, czyli o~27,2~ms, natomiast opóźnienie wariantu Direct -- z~31,3 do~33,8~ms, czyli o~2,5~ms. Przewaga wariantu binarnego osiąga w~tej konfiguracji 43\%: dokument JSON o~rozmiarze 90,5~kilobajta nie mieści się w~jednym oknie, natomiast wiadomość binarna o~rozmiarze 60,5~kilobajta -- mieści.

Przy 2000~rekordach przewaga wynosi 30\%, czyli zbliża się do~zmierzonego współczynnika redukcji wolumenu danych. Powyżej kilku okien liczba obiegów staje się w~przybliżeniu proporcjonalna do~rozmiaru odpowiedzi, a~przewaga czasowa zbiega wówczas do~przewagi objętościowej.
...
Przy małych ładunkach zmienność wynikająca z~zimnej pamięci podręcznej przewyższa różnicę wynikającą z~protokołu, przez co ta~grupa nie stanowi użytecznego narzędzia porównawczego. Do~izolowania wpływu protokołu służy scenariusz Echo, natomiast do~oceny zachowania w~warunkach realnych -- scenariusz z~ciepłą pamięcią podręczną.
...
Ścieżka bezpośrednia korzysta z~HTTP/2 i~TLS, a~badany wariant REST z~HTTP/1.1. Wariant z~Envoy przenosi ładunek binarny, używając HTTP/1.1 na~odcinku pomiędzy przeglądarką a~proxy. Różnica REST--Envoy obejmuje obecność proxy i~odmienny sposób obsługi komunikatów. Różnica Envoy--Direct obejmuje natomiast jednocześnie pominięcie proxy, TLS oraz zmianę wersji HTTP. Zestawienie nie wyodrębnia przyczynowego udziału każdego z~tych czynników.
...
\hline
\textbf{Konfiguracja} & \textbf{REST} & \textbf{Envoy} & \textbf{Direct} & \textbf{Envoy--REST} & \textbf{Direct--Envoy} \\
\hline
...
W~konfiguracji Products, PS=500 i~VU=10 różnica REST--Direct wynosi 24,7~ms. Z~tej wartości 23,5~ms stanowi różnicę REST--Envoy, a~1,2~ms różnicę Envoy--Direct. Różnica REST--Envoy odpowiada zatem około 95\% całkowitej różnicy REST--Direct w~tej konkretnej konfiguracji. Wynik wskazuje na~znaczenie formatu i~wolumenu odpowiedzi, lecz nie pozwala wyznaczyć liczbowego udziału samej serializacji.
...
Zasadnicze znaczenie ma~jednak zestawienie różnic pomiędzy protokołami w~układzie sparowanym. Wartość dodatnia oznacza, że~pierwszy z~wymienionych protokołów jest wolniejszy. Kolejne akapity omawiają wyniki dla dużych i~małych odpowiedzi, wariantów gRPC-Web oraz scenariuszy zapisu.
...
W~zakresie małych ładunków obraz jest bardziej złożony, niż wskazywałaby sama wielkość różnicy. W~scenariuszu Products przy 200~produktach różnica REST--Direct wynosi $1{,}4 \pm 2{,}5$~ms, a~zatem nie jest rozstrzygnięta. W~scenariuszu Echo przy tej~samej liczbie rekordów różnica wynosi $1{,}1 \pm 0{,}7$~ms i~pozostaje bez znaczenia praktycznego wobec opóźnienia obiegu pakietu wynoszącego 26,2~ms.
...
W~scenariuszach zapisu żadna z~różnic nie jest rozstrzygnięta. Przedziały są~w~tej grupie od~kilku do~kilkudziesięciu razy szersze niż w~odczycie. Jedyny wyjątek dotyczy pary Envoy--Direct przy pięćdziesięciu pozycjach i~dwudziestu klientach ($-59{,}1 \pm 45{,}2$~ms), lecz znak tej różnicy jest przeciwny niż w~pozostałych komórkach tej grupy, a~jej wartość wynika z~jednego odstającego przebiegu.
...
\begin{sloppypar}
\textbf{H1 (warianty gRPC-Web osiągają niższe opóźnienie niż REST, a~przewaga rośnie z~rozmiarem odpowiedzi):} hipoteza potwierdzona z~zastrzeżeniem dotyczącym charakteru zależności. Przewaga występuje i~jest znaczna: 26--43\% przy odpowiedziach od~500~rekordów. Wzrost nie ma~charakteru ciągłego. Efekt ujawnia się po~przekroczeniu rozmiaru okna odbiorczego, natomiast w~zakresie do~200~rekordów pozostaje w~granicach kilku procent.
\end{sloppypar}
...
klientach i~dziesięciu rekordach. W~tym zakresie różnice są~praktycznie nieistotne.
\end{sloppypar}

\begin{sloppypar}
\textbf{H3 (wariant gRPC-Web przez Envoy zachowuje przewagę względem REST przy HTTP/1.1 na~odcinku przeglądarkowym):} hipoteza potwierdzona w~zakresie badanych konfiguracji. W~konfiguracji Products, PS=500 i~VU=10 różnica REST--Envoy stanowi około 95\% różnicy REST--Direct. Wynik wskazuje na~znaczenie formatu i~wolumenu odpowiedzi, jednak zastosowany układ nie pozwala oddzielnie oszacować wpływu serializacji, TLS, proxy i~wersji HTTP.
\end{sloppypar}
